...
async function create (req, res) {
  let { bookId } = req.params // Destructure the BookId off params
  let { body } = req.body // Destructure the body off of the req.body

  let book = await BookModel.findById(bookId) // Retrieve the book out of the DB

  book.comments.push({ body }) // Push the body of the comment onto the array of comments
  await book.save() // Await the process to be complete
